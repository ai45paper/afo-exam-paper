...
def extract_text(doc, page_idx):
    try:
        ...
        if text and len(text.split()) > 50:
            return text
        logger.warning(f"Page {page_idx+1} skipped (diagrams/blank).")
        return ""
    except Exception as e:
        logger.error(f"Page {page_idx+1} read error: {e}")
        return ""
...
3. No Repetition: Ensure no previous questions are repeated.
4. Options: Exactly 5 options per question.
5. Answer Match: The 'answer' MUST exactly match the text of one of the options.
6. Format: Return a RAW JSON array. No markdown code blocks.
...
def generate_questions(text, section):
    ...
    providers = [("OpenRouter", call_openrouter), ("NVIDIA", call_nvidia), ("Claude", call_claude), ("Gemini", call_gemini)]
    
    for attempt in range(2):
        for name, func in providers:
            if time.time() - start_time > 120: return None
            try:
                logger.info(f"Trying {name}...")
                raw = func(prompt)
                if raw:
                    logger.info(f"{name} RAW (first 100 chars): {raw[:100].replace(chr(10), ' ')}")
...
def main_workflow():
    ...
    while current < total_pages:
        ...
        if len(combined.strip()) > 50:
            ...
            logger.warning(f"Not enough text on pages {current+1}-{next_page}")

        update_tracker(next_page)
        current = next_page
        gc.collect()
        
        logger.info("Waiting 20 seconds (Rate Limit Protection)...")
        time.sleep(20)
...
@app.route('/')
def home(): return "AGTA 2026 Engine LIVE - OCR Disabled & RAM Safe ✅"
